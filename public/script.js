...
function Load() {
  ...
  loadreq.onreadystatechange = () => {
    if (loadreq.readyState === 4 && loadreq.status === 200) {
      ...
      osszesDiv.innerHTML =
        '<p>Összes recept:</p><button onclick="Load()">Frissítés</button>';
...
async function viewIngredients() {
  ...
  if (!res.ok) {
    document.getElementById("ingredientsList").innerText =
      "Nem található recept vagy hiba történt!";
    return;
  }
...
async function deleteIngredientFromRecipe() {
  ...
  const ingredientToDelete = document
    .getElementById("ingredientToDelete")
    .value.trim()
    .toLowerCase();

  const recipeRes = await fetch(`/recipes/${recipeId}`);
  if (!recipeRes.ok) {
    document.getElementById("ingredientDeleteMsg").innerText =
  ...
  document.getElementById("ingredientDeleteMsg").innerText =
    result.message || "Törlés sikertelen";
}
